...
def figsphere_main(rtp:np.ndarray, Z: int, *, charges: np.ndarray | None = None, done: int=0, ax_main: plt.Axes | None = None, show_convex_hull: bool = True, view= (45, 45), clear_when_done_lt_2: bool = True, sphere_alpha: float =0.4, n_lone_pairs: int =0, PE_values: np.ndarray | None= None, angles: np.ndarray | None = None, target_energy: float | None = None, artists: dict | None= None,) -> Tuple[plt.Figure, plt.Axes, dict]:
    ...
    if artists is None:
        ...
        pot_base_line, = ax_pot.plot([], [], "o-", color="blue", label="Base geometry")
        pot_mod_line, = ax_pot.plot([], [], "o-", color= "orange", label= "lone pairs added")
        pot_tgt_line= (ax_pot.axhline(target_energy, color= 'black', linestyle = "--", label= "Target") 
                       if target_energy is not None 
                       else None
        )
        base = PE_values[0, :] if(PE_values is not None and PE_values.shape[0] >= 1) else np.array([])
        mod = PE_values[1, :] if (PE_values is not None and PE_values.shape[0] >= 2) else np.array([])
        base = base[base != 0]
        mod = mod[mod !=0]
        ...
        ax_pot.set_xlim(0, max(100, x_base.size, x_mod.size))
        ...
        if mod.size:
            handles.append(pot_mod_line); labels.append("LP added")
        ...
        ax_text= fig.add_axes([.80, .01, .17, .04]); ax_text.axis("off")
        ax_text.axis("off")
        current_pe= None
        if n_lone_pairs and mod.size: 
            current_pe= float(mod[-1])
        elif base.size:
            current_pe= float(base[-1])
        
        if current_pe is not None:
            display_text = f"Current PE: {current_pe:.3f}"
        else: 
            display_text = "Current PE: N/A"
        current_pe_text = ax_text.text(0.0,0.5, display_text, transform=ax_text.transAxes, fontsize=10, verticalalignment="center", horizontalalignment="left", bbox=dict(facecolor="white", edgecolor="none", alpha=.8),)
        ...
        plt.pause(0.001)
        artists.update(dict(fig=fig, ax_main= ax_main, ax_pot=ax_pot, ax_text = ax_text, ax_tbl = ax_tbl, surf= surf, rays= rays, pts= pts, labels= labels_txt, hull=hull_tri if ('hull_tri' in locals()) else None, pot_base= pot_base_line, pot_mod=pot_mod_line, pot_tgt=pot_tgt_line, tbl=tbl, geom_txt= geom_txt, current_pe_text=current_pe_text, Z=Z, charges= charges.copy()))
        ...
        ax_pot = artists["ax_pot"]
        ax_pot.set_xlim(0, max(100, x_base.size, x_mod.size))
     
        handles, labels = [], []
        if base.size:
            handles.append(artists["pot_base"])
            labels.append("base geometry")
        if mod.size:
            handles.append(artists["pot_mod"])
            labels.append("LP added")
        old_leg = ax_pot.get_legend()
        if old_leg:
            old_leg.remove()
        if handles:
        ...
        ax_text = artists["ax_text"]
        current_pe_text = artists.get("current_pe_text")
        current_pe = None
        if n_lone_pairs and mod.size:
            current_pe = float(mod[-1])
        elif base.size:
        ...
        if current_pe is not None:
            ...
            display_text = "Current PE: N/A"
        if current_pe_text is None:
            current_pe_text = ax_text.text(0.0, 0.5, display_text, fontsize=10, verticalalignment="center", horizontalalignment="left", bbox=dict(facecolor="white", edgecolor="none", alpha=.8),)
            artists["current_pe_text"] = current_pe_text
        else:
            current_pe_text.set_text(display_text)
        if angles is not None:     
            if Z!= 4:
                A= np.round(angles, 0).astype(int).astype(str) 
            else:
