...
def _energy_pad(pe_trace, N=100):
    pe = np.asarray(pe_trace, dtype=float)
    PE= np.zeros((2, N), dtype=float)
    if pe.size :
        n= min(pe.size, N)
        PE[0, :n]= pe[:n]
    return PE
...
class VSEPR_UI:
    ...
    def __init__(self, z0: int =4, seed: int =42, energy_window: float= 1e-8, show_moves: int =30):
        ...
        for ax in (self.ax_main,):
            ...
            if _HAS_DROPDOWN:
                self.ax_z= self.fig.add_axes([panel_left, 0.86, 0.20, 0.05])
                self.ddown = Dropdown(self.ax_z, label = "Z", options=["2", "3", "4", "5", "6"], value = str(z0))
                self.ddown.on_select(self._on_select_Z)
                self._z_widget = self.ddown
            else:
                self.ax_z = self.fig.add_axes([panel_left, .75, .20, .16])
            ...
            self.ax_lp = self.fig.add_axes([.56, 0.86, 0.20, 0.12])
    ...
    def _on_select_Z(self, selection):
        ...
        self.lp_mask = np.zeros(self.Z, dtype= bool)
        self._rebuild_lp_panel()
        self._log(f"Selected Z = {self.Z}. Click Solve to re-run")
    
        self.lp_mask = np.zeros(self.Z, dtype= bool)
        self._rebuild_lp_panel()
        self._log(f"Selected Z = {self.Z}. Click Solve to re-run")
    def _rebuild_lp_panel(self):
        try:
            self.ax_lp.remove()
            self.ax_apply_lp.remove()
        except Exception:
            try:
                self.ax_lp.remove()
                self.ax_apply_lp.remove()
                    ...
                    try:
                        self.fig.add.delaxes(self.ax_lp)
                        self.fig.add.delaxes(self.ax_apply_lp)
                    except Exception:
                        pass 
            panel_left = .78
            self.ax_lp = self.fig.add_axes([panel_left, 0.53, 0.20, 0.12])
            self.ax_lp.set_title("Lone Pairs", fontsize= 9)
            lp_labels = [f"P{i+1}" for i in range(self.Z)]
            self.chk_lp = CheckButtons(self.ax_lp, labels = lp_labels, actives= [False]*self.Z)
            self.chk_lp.on_clicked(self._on_toggle_lp)
    ...
    def _live_on_update(self, state:dict):
        if self._frames < self.show_moves:
            ...
            _, _, self.artists = figsphere_main(state["rtp"], state["Z"], charges= state["charges"], done= state["done_flag"], ax_main= self.ax_main, n_lone_pairs=n_lp, PE_values= state["PE_values"], angles = state["angles"],target_energy= state["target_energy"], artists= self.artists,)
            self.fig.canvas.draw_idle()
            plt.pause(0.001)
            self._frames +=1
